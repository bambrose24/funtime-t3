...
Schema changes ship as checked-in SQL under `packages/api/prisma/migrations/`. Production uses the `funtime_db` Postgres schema via `?schema=funtime_db` on database URLs. Deployments do not run migrations automatically.
...
Cron services do not run migrations.
...
5. Review the deployment, then run `pnpm db:migrate:deploy` manually against the target production database.
