...
- Delivery: pushed and opened [PR #38](https://github.com/bambrose24/funtime-t3/pull/38) as a draft after explicit user approval. The earlier automatic push-approval blocker is resolved. CI is not being monitored, per the agreed workflow.
...
- Task ID: `P6-TEST-E2E-INFRA-001`
  ...
  - Scope touched: `docs/E2E_INFRA_PLAN.md`, `E2E_INFRA_WORKLOG.md`, `supabase/config.toml`, `supabase/fixtures/season-2025.json`, `supabase/seed.sql`, `scripts/e2e/*`, `.github/workflows/mobile-e2e-supabase.yml`, `package.json`, shared season constants in API/mobile/web, and testing/parity docs.
  - Outcome: Added a separate E2E infra project/worklog, shared season abstraction (`CURRENT_SEASON`) across API/mobile/web, deterministic `teams+games` seed pipeline (with optional live refresh + future timestamp transform), one-command backend bootstrap/verification scripts, initial Maestro smoke flows, and a manual+nightly CI scaffold for local Supabase + Android emulator E2E.
