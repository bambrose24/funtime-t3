...
describe("season renewal eligibility", () => {
  it("opens self-service renewals for the 2026 season", () => {
    expect(canCreateNextSeasonLeagues(2025)).toBe(false);
    expect(canCreateNextSeasonLeagues(2026)).toBe(true);
  });

  it("requires the prior league to be completed once renewals are open", () => {
    expect(
      getRenewalIneligibilityReason(
        { season: 2025, status: "in_progress" },
        { targetSeason: 2026, renewalsOpen: true },
      ),
    ).toBe("Only completed leagues can be renewed");

    expect(
      getRenewalIneligibilityReason(
        { season: 2025, status: "completed" },
        { targetSeason: 2026, renewalsOpen: true },
      ),
    ).toBeNull();
  });
