...
describe("resolveDeepLink", () => {
  it("maps required play-funtime.com routes to mobile routes", () => {
    expect(
      resolveDeepLink("https://play-funtime.com/join-league/ABC123"),
    ).toEqual({
    ...
    expect(resolveDeepLink("https://play-funtime.com/league/42/chat")).toEqual({
      href: "/league/42?tab=messages",
      mode: "replace",
    });
  ...
  it("preserves league query parameters such as tab selection", () => {
    expect(
      resolveDeepLink("https://play-funtime.com/league/42?tab=picks"),
    ).toEqual({
    ...
    expect(
      resolveDeepLink(
        "https://play-funtime.com/league/42?tab=leaderboard&week=1",
      ),
    ).toEqual({
  ...
  it("normalizes auth callback links and requires a code", () => {
    expect(
      resolveDeepLink("https://play-funtime.com/auth/callback?code=abc123"),
    ).toEqual({
    ...
    expect(
      resolveDeepLink(
        "https://play-funtime.com/auth/callback?code=abc123&next=/league/99",
      ),
    ).toEqual({
    ...
    expect(
      resolveDeepLink("https://play-funtime.com/auth/callback"),
    ).toBeNull();
  });
  ...
  it("normalizes recovery callback links from hash/session params", () => {
    ...
    expect(
      resolveDeepLink(
        "funtime://confirm-reset-password#access_token=at123&refresh_token=rt456",
      ),
    ).toEqual({
